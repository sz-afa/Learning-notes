...
1. Writer是输出字符流的⽗类，它是一个抽象类:
    ...
    对比BufferWriter,它需要实时查表，效率低，其实缓冲区IO的各个都有，只不过很小被忽略,
    OutputStream都有flush方法，看子类是否有重写
...
# 字符流字节流桥梁
## 背景
1. 计算机存储的单位是字节，从持久设备读取到程序中是解码，从程序写到持久设备中是编码不管是编码还是解码，不同字符集编解码成 字符 需要不同的个数，因此字节流读取容易出错，比如乱码。
